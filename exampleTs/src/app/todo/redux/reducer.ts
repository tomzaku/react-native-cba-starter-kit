...
const reducer = (state: TTodoState = initialState, action: ITodoAction<TTask>) => {
  switch (action.type) {
	...
	case actionType.ADD_TASK: {
		const tasksMerged = from({ [action.payload.id]: action.payload }).merge(state.tasks)
	...
	case actionType.DELETE_TASK: {
		...
		const taskRemoved = from(state.tasks).without(action.payload.id)
		return state.setIn(['tasks'], taskRemoved)
					.setIn(['tasksIndex'], state.tasksIndex.slice(0, taskCurrentIndex)
														.concat(state.tasksIndex.slice(taskCurrentIndex + 1)))
	}
	case actionType.ADD_TAG: {
		const tagsMerged = from({ [action.payload.id]: action.payload }).merge(state.tags)
